...
description: El modelo mental de enu — kernel mínimo, extensiones, el modelo de concurrencia del navegador, workers y la API sagrada.
---
...
El kernel de `enu` solo conoce **sus propias capacidades**: primitivas (runtime,
...
`enu` toma su modelo de concurrencia del navegador y de Luau:
...
enu.task.spawn(function()
  local cfg = enu.fs.read("config.json")   -- ⏸ suspende, devuelve directo
...
[`enu.task.cleanup(fn)`](/enu/api/task/). Los códigos `ECANCELED` y
...
[**worker**](/enu/api/worker/): un estado Lua nuevo en su goroutine, **sin
memoria compartida**. La comunicación es por **paso de mensajes JSON-ables**
(copiados, no referencias). Un worker no tiene `enu.ui` ni el bus de eventos
...
**ninguna activa por defecto**. `enu` recién instalado es un runtime desnudo;
...
`enu --default-config` hace lo mismo de un comando —en ambos casos sin red—.
Mismo modelo mental que Neovim: el programa no trae plugins activados.

Un [**plugin**](/enu/api/plugin/) es un directorio con `plugin.toml`
...
Con esto, ya puedes leer cualquier página de la [referencia](/enu/api/convenciones/).

...
description: Ejecuta Lua con nu -e, entiende el estado principal frente a las tasks y maneja errores estructurados.
---

`nu` es, antes que nada, un runtime de Lua. La vía más rápida de probarlo es
`nu -e`, que **evalúa un chunk Lua sin TTY (headless) e imprime sus valores de
retorno**.
...
En `nu` `print` es un alias de `nu.log.info`: escribe al fichero de log, **nunca
a la pantalla** (la UI es de las extensiones, no del core). Para que algo
aparezca en stdout con `nu -e`, devuélvelo con `return`.
:::
...
El chunk de `nu -e` corre en el **estado principal** (single-threaded, con event
loop). El estado principal **no es una task**, así que **no puede llamar
funciones suspendientes** (las marcadas con ⏸: casi todo el IO —`nu.fs.read`,
`nu.http.request`, `nu.proc.run`…). Si lo intentas:

```sh
nu -e 'return nu.fs.read("README.md")'
```
...
nu.task.spawn(function()
  local texto = nu.fs.read("README.md")   -- ⏸ aquí sí vale
  nu.fs.write(nu.fs.tmpdir() .. "/copia.md", texto)
end)
return "lanzada"
'
```

`nu -e` espera a que **todas** las tasks que lanzó el chunk terminen antes de
...
```sh
nu -e '
local ok, err = pcall(function() return nu.json.decode("{roto") end)
return ok, nu.json.encode(err)
'
```
...
`nu -e` sale con **0** si el chunk no lanzó, y con **1** si lanzó (un error sin
capturar). Útil en scripts y CI:

```sh
nu -e 'assert(nu.version.api >= 2)' && echo "API suficiente"
```

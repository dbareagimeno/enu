...
que vive bajo el global `nu` y solo crece por adición. Lo que **no** está aquí
(toolkit de widgets, agente, chat, MCP, providers) es una extensión y se versiona
aparte.

## Notación de firmas

Las firmas usan la notación `nu.mod.fn(arg: tipo, opts?: tabla) -> tipo`:
...
Toda la API vive bajo el global `nu`, con submódulos. `require` queda reservado
...
loader. Y `print` está **redirigido a `nu.log.info`** (va al log, no a la

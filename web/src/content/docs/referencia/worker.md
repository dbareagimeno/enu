...
**no existen** `nu.ui`, `nu.events` (el bus principal) ni workers anidados; sí el
...
local w = nu.worker.spawn("mi-plugin/analizador", {
  caps = { "fs.read", "json" },
})
```
...
nu.task.spawn(function()
  local w = nu.worker.spawn("mi-plugin/worker")
...
timers y futures (todo `nu.task` [W]). **Sin watchdog**: los workers existen para

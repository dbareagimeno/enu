...
// del patrón estrangulador (DM2: `nu.toml [vm] backend` + env `NU_VM`) se retiró
...
// `nu.toml [vm] backend` piden todavía el backend `gopher`, ya retirado (M17). NO
// rompe el arranque: nu ignora la petición y sigue sobre wasm. Es la tolerancia
...
func warnIfGopherRequested(tomlBackend string) {
	...
	if req == "gopher" {
		fmt.Fprintln(os.Stderr,
			"nu: el backend gopher se retiró (M17); nu corre siempre sobre wasm")
	}
}

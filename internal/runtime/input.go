...
// x?, y?, text?, path?}`. Para un paste de imagen (G30) el driver trae los bytes en
// `pasteBytes` con `pasteIsText=false`; el backend wasm los vuelca a `nu.fs.tmpdir` y
// entrega el evento con `path` en vez de `text` —los bytes binarios nunca cruzan a Lua—.
type inputEvent struct {
	...
	// trae los bytes en `pasteBytes` con `pasteIsText=false`.
	text        string
	path        string
	pasteBytes  []byte
	pasteIsText bool

...
1. **El puntero** (esta misma línea, la única imperativa del documento):
   ...
   > **▶ Próxima sesión: `S10`** · Fase 2 · S09 cerrada (**watchdog de slice**: presupuesto por slice 100 ms por defecto, configurable vía `WithSliceBudget` —el gancho que S11/S12 cablearán a `nu.toml`—; aborto por `EBUDGET` **NO capturable** reusando el desenrollado de S08 con `reason = abortBudget`; `Task:await` de una abortada por watchdog observa `EBUDGET`; emisión de `core:plugin.misbehaved` por un **gancho interno** `rt.emitMisbehaved` que hoy loguea best-effort y que **S10 cableará** a `nu.events.emit(...)`). **CP-2 verde → Fase 1 cerrada.** El corte de un slice de **CPU puro** que no suspende se logra con `LState.SetContext`: cada thread de task lleva un `context.Context` propio que el intérprete de gopher-lua v1.1.2 vigila en cada instrucción (`mainLoopWithContext`); el watchdog —en su propia goroutine, sin el token— lo cancela al exceder el presupuesto, rompiendo incluso `while true do end`, y la goroutine de la task "reclama" el corte como aborto no capturable (flag atómico `budgetExceeded` → `aborting`/`reason`/`canceled` los pone la propia task bajo el token, invariante de S08 intacto). **Sin hallazgo ni veto:** gopher-lua v1.1.2 SÍ permite interrumpir un slice de CPU puro de forma integrable con el desenrollado de S08; el hito de veto S08+S09 (ADR-008) queda validado. El scheduler sigue **sin yields de corrutina** (goroutine-por-task + token Lua, **ADR-011**, grieta **G31**). S10 (`nu.events`) depende de S04; **cableará** el gancho `emitMisbehaved` a un `emit("core:plugin.misbehaved", ...)` real. Está en el inventario 🔒.

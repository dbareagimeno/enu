...
| Pilar 2: `enu env lock/verify/reproduce` (entorno completo) | No existe; el lockfile de Fase 2 cubre solo plugins | NUEVO → TENSIÓN T4 | **P48** (comprometida) |
...
| Pilar 3: **enforcement por plugin en la frontera Go** | ADR-008: aislamiento por tarea; los plugins comparten el estado principal; la valla dura son los workers con `caps` (guia-plugins.md §5) | CHOCA-CON-ADR → TENSIÓN T1 | **P46** |
...
| Pilar 3: revocación de permisos, auditoría de decisiones | Caps inmutables al spawn (ADR-024); el audit-log encaja en `trace` (Fase 3) | NUEVO (menor) | Diseño Fase 2/3; registrado en P46 |
...
| Producto cotidiano: cancelación, reintentos/rate-limits, cambio de modelo, compactación, subagentes, recuperación | Ya en agente.md (§8, G19, G42/G43, P22–P25 implementadas); G60 abierta (lock huérfano) | YA-HECHO (contratos) | — |
...
| Interop: **ACP como servidor oficial** | Cero menciones en docs/ (ni ADR ni P##) — hueco genuino | NUEVO → TENSIÓN T3 | **P47** |
...
| Observabilidad: suite de evaluación del harness (el loop) | Nada la cubre (`/salud` y `/juicio` evalúan código, no el loop) | NUEVO | **P51** |
| Fiabilidad: crash de escritura de sesión / lockfile | JSONL append-only + `exclusive`; pero G60 (lock huérfano) es exactamente este caso, ya abierta | YA-DECIDIDO (G60 prerrequisito de Fase 3) | Kill-tests como criterio de aceptación de G60 |
...
| Fiabilidad: fault injection | No existe arnés (429/500, disco lleno, fs read-only, workers muertos); SEC-01 (panic en HostFn) sigue sin dueño formal | NUEVO | **P52** |
| Fiabilidad: soak tests | No existen; el estrés de `-race` de `/salud` no es soak de sesión real | NUEVO | **P53** |
...
| Contratos: deprecación, ventanas, LTS, política de soporte, tooling de migración | Hueco genuino sin P## | NUEVO → TENSIÓN T8 | **P49** |
| Contratos: conformance suite ejecutable para terceros | La conformance conceptual existe (rondas + 🔎 + 🔒); el artefacto ejecutable para un autor externo, no | NUEVO → TENSIÓN T8 | **P50** |
...
| Gobernanza: CLA «caso por caso» genera desconfianza | Cierto textualmente (CONTRIBUTING.md §Titularidad) | TENSIÓN T6 | **ADR-029: DCO** |
...
- **T1 — Capacidades por plugin.** El manifiesto declarativo + verificación en
  ...
  como **P46** con disparador calcado al de P2, para que no entre por la vía
...
- **T3 — ACP.** Nueva pospuesta **P47**: adaptador ACP sobre el RPC de Fase 3,
...
- **T4 — `enu env` (reproducibilidad del entorno completo).** Nueva pospuesta
  **P48**, con un matiz dictado por el operador: **no es opcional** — «es una
  feature que tiene que estar sí o sí». El disparador (Fase 2 estable) fija el
  *cuándo*, no el *si*; P48 queda marcada como **comprometida**.
...
- **T6 — Contribuciones.** Se abandona el «caso por caso»: la política estable
  ...
  `Signed-off-by`). Decidido en **ADR-029**; CONTRIBUTING.md §Titularidad se
...
- **T7 — Idioma del frente de gobernanza.** CONTRIBUTING.md pasa a inglés, se
  ...
  con ADR-025 pieza 5.
- **T8 — Compatibilidad y 1.0.** Dos pospuestas nuevas: **P49** (régimen de
  ...
  **P50** (conformance suite ejecutable para terceros; disparador: el primer
...
como **P51/P52/P53** con disparadores propios (timing distinto: fault
...
| Artefacto | Contenido |
|---|---|
| **P46** | Enforcement de capacidades por plugin en la frontera Go (la parte de la propuesta que choca con ADR-008) |
| **P47** | Adaptador ACP (Agent Client Protocol) sobre el RPC de Fase 3 |
| **P48** | `enu env`: reproducibilidad del entorno completo del harness — **comprometida** |
| **P49** | Régimen de compatibilidad post-1.0 (deprecación, LTS, soporte, migración) |
| **P50** | Conformance suite ejecutable para terceros (plugins y providers) |
| **P51** | Suite de evaluación del comportamiento del harness (el loop del agente) |
| **P52** | Arnés de fault injection contra el binario |
| **P53** | Soak tests de sesiones largas |
...
ADR-008; queda registrado en P46, no ejecutado); las extensiones por editor
...
1. **«Solo añadir para siempre acumula decisiones que nunca se pueden
   ...
   real y queda registrada como P49.
